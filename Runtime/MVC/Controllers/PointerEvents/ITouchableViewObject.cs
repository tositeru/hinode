...
{
    /// <summary>
    /// PointerEventDispatcherに対応したIViewObjectを表します。
    ///
    /// System.IComparable<ITouchableViewObject>では以下の優先順位でソートされます。
    ///   - Canvas(ScreenOverlay)
    ///     - 画面前面にあるもの
    ///   - Other
    ///     - Raycastの順番
    /// </summary>
    public interface IOnPointerEventControllerObject
    {
    ...
    /// <summary>
    /// ITouchableViewObjectを以下の優先順位でソートします。
    ///   - Canvas(ScreenOverlay)
    ...
    /// </summary>
    public class TouchableViewObjectComparer : IComparer<IOnPointerEventControllerObject>
    {
        public Camera UseCamera { get; }
        public TouchableViewObjectComparer(Camera useCamera)
        {
            UseCamera = useCamera;
        }
